...
impl XRefs {
                ...
                ir::Operation::Call(call) => {
                    if let Some(arguments) = call.arguments() {
                        for argument in arguments {
                            let argument_value = argument.constant()
                                .and_then(|constant| constant.value_u64());
                            if let Some(value) = argument_value {
                                xrefs.insert(instruction.address().unwrap(),
                                             value);
                            }
                        }
                    }
    ...
    pub fn from_to(&self) -> &HashMap<u64, HashSet<u64>> { &self.from_to }
    pub fn to_from(&self) -> &HashMap<u64, HashSet<u64>> { &self.to_from }
