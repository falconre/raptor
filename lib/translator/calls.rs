...
use data::{FUNCTIONS, ParameterType};
use error::*;
...
pub fn no_returns(function: &mut ir::Function<ir::Constant>) -> Result<()> {
                    ...
                    ir::Operation::Call(call) =>
                        if call.target()
                               ...
                               .map(|function_declaration|
                                    function_declaration.no_return())
                               .unwrap_or(false) {
                            no_returns.push(
                                ir::FunctionLocation::Instruction(
                                    block.index(),
                                    instruction.index()));
                        },
                    _ => continue
    ...
    for no_return in no_returns {
        ...
        if function.block(no_return.block_index().unwrap())
                   .unwrap()
                   .instructions()
                   .last()
                   .unwrap()
                   .index() == no_return.instruction_index().unwrap() {
            let edges_out =
...
pub fn call_sites<'f>(
    ...
    for call_site in call_sites {
        ...
        for location in call_site.set_return_address() {
            ...
            let mut block = new_function.block_mut(block_index)?;
            block.replace_with_nop(location.instruction().unwrap().index())?;
        }

        let block_index = call_site.branch().block().unwrap().index();
        let mut block = new_function.block_mut(block_index).unwrap();
        let instruction_index =
            call_site.branch().instruction().unwrap().index();
        let mut instruction = block.instruction_mut(instruction_index).unwrap();
        ...
        let mut call =
            ir::Call::new(ir::CallTarget::Expression(target_expression));
    ...
    for return_site in return_sites {
        ...
        let mut block = new_function.block_mut(block_index)?;
        let mut instruction = block.instruction_mut(
            return_site.branch().instruction().unwrap().index()).unwrap();
...
pub fn get_argument_variables(
    ...
    for i in 0..arguments.len() {
        match translation_information.calling_convention().argument_type(i) {
            ...
            ArgumentType::Stack(offset) => {
                ...
                // for that.
                if translation_information.calling_convention()
                             .return_address_type()
                             .stack()
                             .is_some() {

                    let word_size = translation_information.architecture().word_size();
                    expr = ir::Expression::sub(expr,
                        ir::expr_const((word_size / 8) as u64, word_size as usize))?;
                }

                let expr =
                    ir::reduce(
                        &stack_pointer_offsets.replace(&expr, word_size)?)?;

                if let Some(stack_pointer) = expr.stack_pointer() {
                    result.push(ir::StackVariable::new(
                        stack_pointer.offset(),
                        translation_information.calling_convention()
                            .stack_argument_length() * 8).into());
                }
                else {
    ...
    for block in function.blocks_mut() {
        ...
        for instruction in block.instructions_mut() {
            ...
            let program_location =
                ir::ProgramLocation::new(function_index, function_location);
            let stack_pointer_offset =
                &stack_pointer_offsets[&program_location];

            let mut call = match instruction.operation_mut() {
            ...
            if let Some(expression) = call.target().expression().cloned() {
                ...
                if translation_information.symbol(address).is_some() {
                    *call = ir::Call::new(
                        ir::CallTarget::Symbol(
                            translation_information.symbol(address)
                                .unwrap()
                                .name()
                                .to_string()));
                    call.set_variables_written(Some(
                        translation_information.calling_convention().trashed_registers()
                            .iter()
                            .map(|scalar| scalar.clone().into())
                            .collect()));
                }
...
pub fn get_parameter(
    ...
    match translation_information.calling_convention().argument_type(position) {
        ArgumentType::Register(register) => register.into(),
        ArgumentType::Stack(offset) =>
            ir::StackVariable::new(
                offset as isize,
                translation_information.calling_convention().stack_argument_length()
            ).into()
    }
}
...
pub fn set_function_parameters<V: ir::Value>(
    ...
    let mut remaining_parameters =
        initial_parameters.into_iter()
