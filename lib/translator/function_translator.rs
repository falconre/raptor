...
impl<'t> FunctionTranslator<'t> {
    ...
    pub fn stack_pointers<'f>(
        ...
        for ref mut block in new_function.blocks_mut() {
            let ref_block =
                function.block(block.index()).expect("Failed to get block");
            for ref mut instruction in block.instructions_mut() {
                
                let ref_instruction =
                    ref_block.instruction(instruction.index())
                    ...
                    match operation {
                        ir::Operation::Store { index, src } => {
                            if let Some(stack_pointer) = index.stack_pointer().cloned() {
                                ir::Operation::Assign {
                                    dst: ir::StackVariable::new(stack_pointer.offset(),
                                                                src.bits()).into(),
                                    src: src.clone()
                                }
                            }
                        ...
                        ir::Operation::Load { dst, index } => {
                            if let Some(stack_pointer) = index.stack_pointer().cloned() {
                                ir::Operation::Assign {
                                    dst: dst.clone(),
                                    src: ir::StackVariable::new(stack_pointer.offset(),
                                                                dst.bits()).into()
                                }
                            }
                            else {
    ...
    pub fn constants(
        ...
        let constants =
            analysis::constants::constants(&function, Some(self.ti().backing()))?;
    ...
    pub fn remove_outgoing_transient_assignments(
            ...
            function.locations()
                ...
                .filter(|location|
                    location.instruction()
                        .map(|instruction|
                            instruction.operation().is_return())
                        .unwrap_or(false));

        let returns =
            returns.map(|function_location|
                ir::RefProgramLocation::new(function, function_location));

        // Get the transient assignments
        let transient_assignments =
            transient_assignments::transient_assignments(function)?;
        ...
        for return_location in returns {
            ...
            'chain: for (variable, tac) in chains {
                ...
                if tac.assignment()
                      .variable()
                      .map(|v| v != variable).unwrap_or(true) {
                    continue;
                }
                ...
                // We can safely nop all instances of this transient variable
                {
                    ...
                    let instruction_index =
                        last_assignment
                            ...
                            .instruction_index()
                            .unwrap();
                    let instruction = block.instruction_mut(instruction_index)
                        .ok_or(format!("Could not get mutable instruction for \
                            function name: {},
                            block index: {},
                            instruction index: {}",
                            function.name(),
    ...
    pub fn strided_intervals(&self, function: &ir::Function<ir::Constant>)
        -> Result<(ir::Function<ir::Constant>,
                   HashMap<ir::ProgramLocation,
                           analysis::strided_intervals::State>)> {
        ...
        for block in new_function.blocks_mut() {
            ...
            for instruction in block.instructions_mut() {
                let fl = ir::FunctionLocation::Instruction(
                    block_index,
                    instruction.index());
    ...
    pub fn translate_function(&self, function: &il::Function)
        -> Result<ir::Function<ir::Constant>> {

        let function =
            ir::Function::<ir::Constant>::from_il(function)
                .expect("Failed to translate function");
    ...
    pub fn optimize_function_inner(
        ...
        loop {
            ...
            let stack_pointer_offsets =
                analysis::stack_pointer_offsets::stack_pointer_offsets(
                    &function_1,
                    self.ti().architecture()
                )?;

            let mut function_2 =
                self.stack_pointers(&function_1, &stack_pointer_offsets)?;

            self.reduce_function(&mut function_2)?;
            self.stack_variables(&mut function_2);

            calls::apply_functions(
                &mut function_2,
                self.ti(),
