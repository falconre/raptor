...
impl<V: Value> Call<V> {
    ...
    pub fn variables_read(&self) -> Option<Vec<&Variable>> {
        if let Some(arguments) = self.arguments() {
            let mut variables: Vec<&Variable> = arguments
                .iter()
    ...
    pub fn target(&self) -> &CallTarget<V> {
        &self.target
    }
    pub fn target_mut(&mut self) -> &mut CallTarget<V> {
        &mut self.target
    }
...
impl<V: Value> fmt::Display for Call<V> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self.target() {
            CallTarget::Expression(expression) => write!(f, "call({})", expression),
            CallTarget::Symbol(string) => match self.arguments() {
                Some(arguments) => write!(
                    f,
                    "{}({})",
                    string,
                    arguments
            ...
            CallTarget::FunctionId(function_id) => write!(f, "id_0x{:x}()", function_id),
        }
    }
}
