...
impl<V: Value> Dereference<V> {
    ...
    pub fn bits(&self) -> usize { self.expression.bits() }
    pub fn expression(&self) -> &Expression<V> { &self.expression }
    pub fn into_expression(self) -> Expression<V> { self.expression }
}

impl<V: Value> Into<LValue<V>> for Dereference<V> {
    fn into(self) -> LValue<V> { LValue::Dereference(self) }
}
