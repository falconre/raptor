...
impl<V: Value> ControlFlowGraph<V> {
    pub fn from_il(control_flow_graph: &il::ControlFlowGraph)
        ...
        control_flow_graph
            ...
            .into_iter()
            .try_for_each(|block|
                graph.insert_vertex(Block::<Constant>::from_il(block)))?;

        control_flow_graph
            .edges()
            .into_iter()
            .try_for_each(|edge|
                graph.insert_edge(Edge::<Constant>::from_il(edge)))?;
    ...
    pub fn graph(&self) -> &Graph<Block<V>, Edge<V>> { &self.graph }
    pub fn graph_mut(&mut self) -> &mut Graph<Block<V>, Edge<V>> {
        &mut self.graph
    }
    pub fn entry(&self) -> Option<usize> { self.entry.clone() }
    pub fn exit(&self) -> Option<usize> { self.exit.clone() }
    ...
    pub fn blocks(&self) -> Vec<&Block<V>> { self.graph.vertices() }
    ...
    pub fn edges(&self) -> Vec<&Edge<V>> { self.graph.edges() }
    pub fn edges_mut(&mut self) -> Vec<&mut Edge<V>> { self.graph.edges_mut() }
