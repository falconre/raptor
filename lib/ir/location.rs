...
impl<'r, V: Value> RefFunctionLocation<'r, V> {
    ...
    pub fn from_address(function: &Function<V>, address: u64)
        -> Option<RefFunctionLocation<V>> {
    ...
    pub fn address(&self) -> Option<u64> {
        self.instruction().and_then(|instruction| instruction.address())
    }
}
...
impl<'r, V: Value> fmt::Display for RefFunctionLocation<'r, V> {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            RefFunctionLocation::Instruction(_, instruction) =>
                instruction.fmt(f),
            RefFunctionLocation::Edge(edge) =>
                edge.fmt(f),
            RefFunctionLocation::EmptyBlock(block) =>
                write!(f, "[ block 0x{:x} ]", block.index())
        }
    }
}
...
impl<'r, V: Value> RefProgramLocation<'r, V> {
    ...
    pub fn address(&self) -> Option<u64> {
        self.function_location.instruction().and_then(|i| i.address())
    }
    ...
    pub fn from_function(function: &Function<V>)
        -> Option<RefProgramLocation<V>> {

        let block = function.block(function.control_flow_graph().entry()?).ok()?;
    ...
    pub fn from_address(program: &Program<V>, address: u64)
        -> Option<RefProgramLocation<V>> {
                    ...
                    if instructions[i].index() == instruction.index() {
                        if i + 1 < block.instructions().len() {
                            return vec![
                                RefProgramLocation::new(
                                    self.function(),
                                    RefFunctionLocation::Instruction(
                                        block,
                                        &instructions[i + 1])
                                )
                            ];
...
impl FunctionLocation {
    ...
    pub fn apply<'f, V: Value>(&self, function: &'f Function<V>)
...
impl ProgramLocation {
    pub fn new(function_index: usize, function_location: FunctionLocation)
        -> ProgramLocation {
    ...
    pub fn apply<'f, V: 'f + Value>(&self, function: &'f Function<V>)
        -> Result<RefProgramLocation<'f, V>> {
        
        Ok(RefProgramLocation::new(function,
                                   self.function_location.apply(function)?))
    }

    pub fn function_index(&self) -> usize { self.function_index }
...
impl<'r, V: Value> From<RefProgramLocation<'r, V>> for ProgramLocation {
    fn from(rpl: RefProgramLocation<'r, V>) -> ProgramLocation {
        ProgramLocation::new(
            rpl.function().index().expect("Could not get function index"),
