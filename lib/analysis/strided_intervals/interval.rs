...
impl Interval {
    ...
    pub fn lo(&self) -> &Value { &self.lo }
    pub fn hi(&self) -> &Value { &self.hi }
    pub fn bits(&self) -> usize { self.lo().bits() }

    pub fn join(&self, other: &Interval) -> Result<Interval> {
        let mut lo = self.lo().min_(other.lo())?;
        let mut hi = self.hi().max_(other.hi())?;

        if lo.sign_overflow(self.lo())? { lo = Value::Top(lo.bits()); }
        if hi.sign_overflow(self.hi())? { hi = Value::Top(hi.bits()); }
    ...
    pub fn widen(&self, other: &Interval) -> Result<Interval> {
        ...
        let lo_cmp = self.lo().binop(other.lo(), |l, r| {
            let exp = 
                ir::Expression::or(l.cmpeq(r)?.into(), l.cmpltu(r)?.into())?;
            Ok(ir::eval(&exp)?)
        })?;

        let lo =
            if lo_cmp.value().map(|value| value.is_one()).unwrap_or(false) {
                self.lo().clone()
            }
            else { Value::Top(self.bits()) };

        // if hi greater than other hi, then keep hi
        let hi_cmp = self.hi().binop(other.hi(), |l, r| {
            let exp =
                ir::Expression::xor(l.cmpltu(r)?.into(), ir::expr_const(1, 1))?;
            Ok(ir::eval(&exp)?)
        })?;

        let hi =
            if hi_cmp.value().map(|value| value.is_one()).unwrap_or(false) {
                self.hi().clone()
            }
            else { Value::Top(self.bits()) };
    ...
    pub fn binop<F>(&self, rhs: &Interval, f: F) -> Result<Interval>
        where F: Clone + Fn(&ir::Constant, &ir::Constant) -> Result<ir::Constant> {
    ...
    pub fn trun(&self, bits: usize) -> Result<Interval> {
        Ok(Interval {
            lo: self.lo().extop(bits, |c: &ir::Constant| Ok(c.trun(bits)?))?,
    ...
    pub fn sext(&self, bits: usize) -> Result<Interval> {
        ...
        if lo.is_signed()? { lo = Value::Top(lo.bits()); }
        if hi.is_signed()? { hi = Value::Top(hi.bits()); }
