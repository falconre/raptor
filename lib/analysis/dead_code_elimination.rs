...
pub fn dead_code_elimination<V: ir::Value>(function: &ir::Function<V>)
    -> Result<ir::Function<V>> {
            ...
            rd.get(&rpl.into())
                ...
                .filter(|location| {
                    ...
                    let cond = !(instruction.operation().is_assign() &&
                      instruction.operation().dst().unwrap().stack_variable().is_some());
                    cond
                })
                .for_each(|location| {
