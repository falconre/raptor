...
pub fn replace_expression<V: ir::Value>(
    ...
    match haystack {
        ...
        ir::Expression::Add(lhs, rhs) =>
            ir::Expression::add(lhs.replace_expression(needle, expr)?,
                            rhs.replace_expression(needle, expr)?),
        ir::Expression::Sub(lhs, rhs) =>
            ir::Expression::sub(lhs.replace_expression(needle, expr)?,
                            rhs.replace_expression(needle, expr)?),
        ir::Expression::Mul(lhs, rhs) =>
            ir::Expression::mul(lhs.replace_expression(needle, expr)?,
                            rhs.replace_expression(needle, expr)?),
        ir::Expression::Divu(lhs, rhs) =>
            ir::Expression::divu(lhs.replace_expression(needle, expr)?,
                             rhs.replace_expression(needle, expr)?),
        ir::Expression::Modu(lhs, rhs) =>
            ir::Expression::modu(lhs.replace_expression(needle, expr)?,
                             rhs.replace_expression(needle, expr)?),
        ir::Expression::Divs(lhs, rhs) =>
            ir::Expression::divs(lhs.replace_expression(needle, expr)?,
                             rhs.replace_expression(needle, expr)?),
        ir::Expression::Mods(lhs, rhs) =>
            ir::Expression::mods(lhs.replace_expression(needle, expr)?,
                             rhs.replace_expression(needle, expr)?),
        ir::Expression::And(lhs, rhs) =>
            ir::Expression::and(lhs.replace_expression(needle, expr)?,
                            rhs.replace_expression(needle, expr)?),
        ir::Expression::Or(lhs, rhs) =>
            ir::Expression::or(lhs.replace_expression(needle, expr)?,
                           rhs.replace_expression(needle, expr)?),
        ir::Expression::Xor(lhs, rhs) =>
            ir::Expression::xor(lhs.replace_expression(needle, expr)?,
                            rhs.replace_expression(needle, expr)?),
        ir::Expression::Shl(lhs, rhs) =>
            ir::Expression::shl(lhs.replace_expression(needle, expr)?,
                            rhs.replace_expression(needle, expr)?),
        ir::Expression::Shr(lhs, rhs) =>
            ir::Expression::shr(lhs.replace_expression(needle, expr)?,
                            rhs.replace_expression(needle, expr)?),
        ir::Expression::Cmpeq(lhs, rhs) =>
            ir::Expression::cmpeq(lhs.replace_expression(needle, expr)?,
                              rhs.replace_expression(needle, expr)?),
        ir::Expression::Cmpneq(lhs, rhs) =>
            ir::Expression::cmpneq(lhs.replace_expression(needle, expr)?,
                               rhs.replace_expression(needle, expr)?),
        ir::Expression::Cmpltu(lhs, rhs) =>
            ir::Expression::cmpltu(lhs.replace_expression(needle, expr)?,
                               rhs.replace_expression(needle, expr)?),
        ir::Expression::Cmplts(lhs, rhs) =>
            ir::Expression::cmplts(lhs.replace_expression(needle, expr)?,
                               rhs.replace_expression(needle, expr)?),
...
impl Constants {
    ...
    pub fn variable(&self, variable: &ir::Variable)
        -> Option<&ir::Constant> {
    ...
    pub fn replace_with_constants(&self, expression: &ir::Expression<ir::Constant>)
    ...
    pub fn reduce(&self, expression: &ir::Expression<ir::Constant>)
        -> Result<ir::Expression<ir::Constant>> {

        ir::reduce(&self.replace_with_constants(expression)?)
    }

    pub fn eval(&self, expression: &ir::Expression<ir::Constant>)
...
impl<'c> ConstantsAnalysis<'c> {
    pub fn new(backing: Option<&'c falcon::memory::backing::Memory>)
        -> ConstantsAnalysis<'c> {
        ConstantsAnalysis {
            backing: backing
        }
...
impl<'r, 'c> fixed_point::FixedPointAnalysis<'r, Constants, ir::Constant>
    ...
    fn trans(&self, location: &ir::RefProgramLocation<'r, ir::Constant>, state: Option<Constants>)
        -> Result<Constants> {
        ...
        let state = match location.instruction() {
            Some(instruction) => match instruction.operation() {
                ...
                ir::Operation::Load { dst, index } => {
                    if let Some(address) = state.eval(&index)?
                                                .and_then(|c| c.value_u64()) {

                        let is_read_only_memory = self.backing.and_then(|backing|
                        ...
                        if is_read_only_memory || is_t9_load {
                            let value = self.backing.and_then(|backing| 
                                backing.get(address, dst.bits())
                                    .map(|constant| Constant::Constant(constant)))
                                    .unwrap_or(Constant::Top);
                ...
                ir::Operation::Intrinsic(intrinsic) => {
                    if let Some(scalars_written) = intrinsic.scalars_written() {
                        scalars_written
                            .into_iter()
                            .for_each(|scalar|
                                state.set_variable(scalar.clone().into(),
                                                   Constant::Top));
                    }
                    else {
                        state.top();
    ...
    fn join<'j>(&self, state0: Constants, state1: &'j Constants)
        -> Result<Constants> {
