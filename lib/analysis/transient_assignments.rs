...
pub fn transient_assignments<'f, V: ir::Value>(function: &'f ir::Function<V>)
    ...
    fn partial_cmp(&self, other: &TransientAssignment) -> Option<Ordering> {
        match self {
            TransientAssignment::Top => match other {
                ...
                TransientAssignment::Variable(_) |
                TransientAssignment::Bottom => Some(Ordering::Less)
            },
            TransientAssignment::Variable(lhs) => match other {
            ...
            TransientAssignment::Bottom => match other {
                TransientAssignment::Top |
                TransientAssignment::Variable(_) => Some(Ordering::Less),
...
impl TransientAssignmentChain {
    ...
    pub fn assignment(&self) -> &TransientAssignment { &self.assignment }
    pub fn chain(&self) -> &[ir::ProgramLocation] {
        &self.chain
    }

    fn join(&self, other: &TransientAssignmentChain)
        -> TransientAssignmentChain {
...
impl PartialOrd for TransientAssignmentChain {
    fn partial_cmp(&self, other: &TransientAssignmentChain)
        -> Option<Ordering> {
...
impl TransientAssignments {
    ...
    pub fn get(&self, variable: &ir::Variable)
        -> Option<&TransientAssignmentChain> {

        self.chains.get(variable)
    }

    pub fn chains(&self)
        -> &HashMap<ir::Variable, TransientAssignmentChain> {

        &self.chains
    }

    fn eval<V: ir::Value>(&self, expression: &ir::Expression<V>)
        -> TransientAssignmentChain {
    ...
    fn top(&mut self) {
        self.chains
            .iter_mut()
            .for_each(|(_, tac)|
                *tac = TransientAssignmentChain::new_top());
    }
}
...
impl PartialOrd for TransientAssignments {
    fn partial_cmp(&self, other: &TransientAssignments)
        -> Option<Ordering> {
                ...
                ir::Operation::Call(call) => {
                    if let Some(arguments) = call.arguments() {
                        arguments.into_iter()
                            ...
                            .for_each(|argument|
                                state.set(argument.clone(),
                                          TransientAssignmentChain::new_top()));
                    }
                    else {
                        state.top();
                    }
