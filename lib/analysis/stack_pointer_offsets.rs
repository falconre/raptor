...
pub fn perfect(
    stack_pointer_offsets:
        &HashMap<ir::RefProgramLocation<ir::Constant>, StackPointerOffsets>,
    stack_pointer: &ir::Variable
) -> bool {
    stack_pointer_offsets
        .iter()
        .all(|(_, stack_pointer_offsets)| 
            stack_pointer_offsets.get(stack_pointer).is_some())
}
    ...
    pub fn replace<V: ir::Value>(
        ...
        match expression {
            ...
            ir::Expression::Ite(cond, then, else_) =>
                ir::Expression::ite(self.replace(cond, bits)?,
...
impl AnalysisValue {
    ...
    fn join(&self, rhs: &AnalysisValue) -> AnalysisValue {
        match self {
            ...
            AnalysisValue::Offset(_) |
            AnalysisValue::Constant(_) =>
                if self == rhs { self.clone() }
                else {
    ...
    fn bits(&self) -> usize {
        match self {
            AnalysisValue::Top(bits) |
            AnalysisValue::Bottom(bits) => *bits,
            AnalysisValue::Offset(constant) |
            AnalysisValue::Constant(constant) => constant.bits()
        }
    }
}


fn eval_binop<F>(lhs: &AnalysisValue, rhs: &AnalysisValue, op: F)
    -> ::error::Result<AnalysisValue>
    where F: Fn(&ir::Constant, &ir::Constant) -> ::falcon::error::Result<ir::Constant> {
...
fn eval(expression: &ir::Expression<AnalysisValue>)
    -> ::error::Result<AnalysisValue> {
...
impl State {
    ...
    fn replace(&self, expression: &ir::Expression<ir::Constant>)
        ...
        match expression {
            ir::Expression::LValue(lvalue) => match lvalue.as_ref() {
                ...
                ir::LValue::Dereference(dereference) =>
                    Ok(ir::Dereference::new(
                        self.replace(dereference.expression())?)
                            .into())
            },
            ir::Expression::RValue(rvalue) => match rvalue.as_ref() {
                ir::RValue::Value(v) =>
                    Ok(ir::value_expr(AnalysisValue::Constant(v.clone()))),
                ir::RValue::Reference(reference) =>
                    Ok(reference.expression().stack_variable()
                        .map(|stack_variable|
                            ir::value_expr(
                                AnalysisValue::Offset(
                                    ir::const_(
                                        stack_variable.offset() as u64,
                                        reference.bits()))))
                        .unwrap_or(
            ...
            ir::Expression::Trun(bits, rhs) =>
                ir::Expression::trun(*bits, self.replace(rhs)?),
            ir::Expression::Zext(bits, rhs) =>
                ir::Expression::zext(*bits, self.replace(rhs)?),
            ir::Expression::Sext(bits, rhs) =>
                ir::Expression::sext(*bits, self.replace(rhs)?),
            ir::Expression::Ite(cond, then, else_) =>
                ir::Expression::ite(self.replace(cond)?,
                                    self.replace(then)?,
                                    self.replace(else_)?)
        }
    }
    ...
    fn variable(&self, variable: &ir::Variable)
        -> Option<&AnalysisValue> {
...
impl<'f> fixed_point::FixedPointAnalysis<'f, State, ir::Constant>
    for StackPointerOffsetAnalysis {
    ...
    fn trans(
        ...
        let mut state = match state {
            ...
            None => {
                // Get function entry
                let function_entry =
                    ir::RefProgramLocation::from_function(location.function())
                ...
                if *location == function_entry {
                    ...
                    state.set_variable(
                        ...
                        AnalysisValue::Offset(
                            ir::Constant::new(0, self.stack_pointer.bits()))
                    );
                    state
                }
                else {
            ...
            ir::RefFunctionLocation::Instruction(_, ref instruction) =>
                match instruction.operation() {
                    ...
                    ir::Operation::Load { dst, index } => {
                        let value =
                            if let Some(index) = eval(&state.replace(index)?)?.offset() {
                                state.load(index).cloned().unwrap_or(AnalysisValue::Top(dst.bits()))
                            }
                            else {
    ...
    fn join(&self, state0: State, state1: &State)
        -> Result<State> {
