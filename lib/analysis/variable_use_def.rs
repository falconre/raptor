use analysis::{LocationSet, use_def};
use error::*;
use ir;
use std::collections::HashMap;


pub fn variable_use_def<'f, V: ir::Value>(function: &'f ir::Function<V>)
    ...
    for location in function.program_locations() {
        let variables =
            location.instruction()
                ...
                .unwrap_or(
                    location.edge().and_then(|edge|
                        edge.condition().map(|c| c.variables())));
